...
if material == "arumi":
	os.system("python lcf_JK2.py " + sorted_file +" "+str(count))
else:
	os.system("python lcf_JK.py " + sorted_file +" "+str(count))
if material == "arumi":
	os.system("python config_JK.py " + sorted_file + " " + str(material) +" "+ str(count))
	os.system("cp arumi.config " +str(count)+ "test.config")
else:
	os.system("python config_JK.py " + sorted_file + " " + str(material) +" "+ str(count))
if args[2] == "water_pillow": ##when using water pillow, ignoring the second path.
	os.system("../../hotspot -f default.flp -c "+str(count)+"test.config -p "+str(count)+"test.ptrace -model_type grid -model_secondary 0 -grid_steady_file tmp.grid.steady"+str(count)+" -detailed_3D on -grid_layer_file "+str(count)+"test.lcf")
else:
...
for i in xrange(0, layer_num):
	if args[2] == "water_pillow": ##the output would be changed whether the second path is used.
		os.system("cat tmp.grid.steady"+str(count)+" | sed -n "+ str(5+i*2*(output_grid_size*output_grid_size+2))+ "," +str(5+i*2*(output_grid_size*output_grid_size+2)+(output_grid_size*output_grid_size-1)) +"p | sort -n -k2 | awk \'END{print $2-273.15}\' >> tmp.results"+str(count))
#		os.system("cat tmp.grid.steady"+str(count)+" | sed -n "+ str(5+i*2*(output_grid_size*output_grid_size+2))+ "," +str(5+i*2*(output_grid_size*output_grid_size+2)+(output_grid_size*output_grid_size-1)) +"p > layer" + str(i+1) + ".grid.steady")
