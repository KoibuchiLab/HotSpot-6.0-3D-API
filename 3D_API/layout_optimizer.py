...
def optimize_power_distribution(layout, per_processor_power_range, total_power_budget, optimization_method, num_random_starts, num_iterations):
	...
	for trial in range(0, num_random_starts):
		[temperature, power_distribution] = minimize_temperature(layout, per_processor_power_range, total_power_budget, optimization_method, num_iterations)
...
def minimize_temperature(layout, per_processor_power_range, total_power_budget, optimization_method, num_iterations):

	if (optimization_method == "basinhopping"):
		return minimize_temperature_basinhopping(layout, per_processor_power_range, total_power_budget, num_iterations)
	elif (optimization_method == "random"):
		return minimize_temperature_random(layout, per_processor_power_range, total_power_budget, num_iterations)
	else:
		sys.stderr.write("Error: Unknown optimizastion method '" + optimizastion_method + "'")

def minimize_temperature_random(layout, per_processor_power_range, total_power_budget, num_iterations):
	...
	while (True):
		...
		reduction = random.uniform(0, min(extra, random_start[victim] - per_processor_power_range[0]))
		random_start[victim]  -= reduction

	sys.stderr.write("Random start: " + str(random_start) + "\n")

	# Compute the temperature
	temperature =  layout_temperature(random_start, [layout, per_processor_power_range])

	return [temperature, random_start]


def minimize_temperature_basinhopping(layout, per_processor_power_range, total_power_budget, num_iterations):
...
[temperature, power_distribution] = optimize_power_distribution(layout, per_processor_power_range, total_power_budget, optimization_method, num_random_starts, num_iterations)
