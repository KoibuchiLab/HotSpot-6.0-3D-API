		...
		for num in numchips:
			for add in add_by:
				...
				for trial in range(1,11):
					...
					command = "mpirun -np 6 ./optimize_layout.py --numchips "+str(num)+" --medium air --chip base3 --diameter 7 --layout_scheme random_greedy:15:5000:"+str(add)+"  --numlevels 7 --powerdistopt uniform_discrete --powerdistopt_num_iterations 1 --powerdistopt_num_trials 1  --overlap .20 --max_allowed_temperature 100  --verbose 0 --mpi"+export_path+str(num)+"_chip_add_"+str(add)+"_trial_"+str(trial)+".pdf --test"
					start = time.time()
