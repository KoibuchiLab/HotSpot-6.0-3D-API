...
# draw the color scale bar	
	...
	my $i;
	my $txt_ymin;
	my $w2=$max_x*0.05*1.75; # toto added *1.75 here
	my $h2=$max_y*0.025*1.75; # toto added *1.75 here
...
#			$level=int(($max_t-(@{$_}[5]))/($max_t-$min_t)*($num_levels-1));
			...
			if ($i%3==0) {
				...
				'" fill="black" text_anchor="start" style="font-size:2000" > '. $scale_value .' </text>'."\n";
			}
		}
		$clr_ymin+=$h2;
	}
	$toto_tmp=$min_t-273.15;
	$toto_tmp=~s/^(\d+)\.(\d)(\d)\d+/$1\.$2$3/;
	$fp.="\t".'<text x="'.$scale_xmin .'" y="'. $clr_ymin .
	'" fill="black" text_anchor="start" style="font-size:2000" > '. $toto_tmp .' </text>'."\n";
	}
