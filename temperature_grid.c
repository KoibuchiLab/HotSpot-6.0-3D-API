...
//	printf("%d %f %f %f\n",i1, model->rows-tolerant_ceil(tu/ch),tu,ch);
...
void parse_layer_file(grid_model_t *model, FILE *fp)
{
  ...
  while (!feof(fp) && count < (inner_layers * LCF_NPARAMS)) {
      ...
      switch (field)
		...
		printf("width model: %f sum: %f \n",model->width, get_total_width(model->layers[i].flp));
		printf("height model: %f sum: %f \n",model->height, get_total_height(model->layers[i].flp));
...
//orignal comment out
  int l;
  fprintf(fp, "%dth is the top silicon\n", silidx);
...
// which layer mean
  int extra_nodes;
  int model_secondary = model->config.model_secondary;
  int n, nl = model->n_layers;
  int spidx, hsidx, intidx, c4idx, metalidx, subidx, solderidx, pcbidx;
  ...
  if (model_secondary) {
      ...
      intidx = LAYER_INT;
       ...
       if (!model_secondary) {
          /* default set of layers	*/
          if (!model->has_lcf) {
              fprintf(fp, "sec:none lcf:none\nsilicon:%d iface:%d hsp:%d hsink:%d \n", silidx, intidx, spidx, hsidx);
          } else {
              fprintf(fp, "sec:none lcf:yes\nhsp:%d hsink:%d and other layers\n", spidx, hsidx);
          }
      } else {
          /* default set of layers	*/
          if (!model->has_lcf) {
              fprintf(fp, "sec:yes lcf:none\nsilicon:%d iface:%d hsp:%d hsink:%d metal:%d c4:%d sub:%d solder:%d pcb:%d \n", silidx, intidx, spidx, hsidx, metalidx, c4idx, subidx, solderidx, pcbidx);
              /* layer configuration file	*/
          } else {
              fprintf(fp, "sec:yes lcf:yes\nhsp:%d hsink:%d sub:%d solder:%d pcb:%d and other layers \n",spidx, hsidx, subidx, solderidx, pcbidx);
          }
      }

  for(l=0;  l < model->n_layers; l++){
	fprintf(fp, "%dth layer\n", l);
      for(i=0; i < model->rows; i++){
          for(j=0; j < model->cols; j++){
              fprintf(fp, "%d\t%.2f\n", i*model->cols+j,
